...
class RecoverAccountPrivateKey extends Component {
    state = {
    ...
    handleSubmit = async () => {
        if (!this.isLegit) {
            Mixpanel.track('IE-SP Recover private key link not valid');
            return false;
        }
        ...
        await Mixpanel.withTracking('IE-SP Recovery with private key',
            async () => {
    ...
    render() {
        ...
        return (
            <StyledContainer className='small-centered border'>
                <h1><Translate id='recoverPrivateKey.pageTitle' /></h1>
                <h2><Translate id='recoverPrivateKey.pageText' /></h2>
                <form onSubmit={(e) => {
                    this.handleSubmit();
                    e.preventDefault();
                }} autoComplete='off'>
                    <RecoverAccountPrivateKeyForm
                        {...combinedState}
...
const RecoverAccountPrivateKeyWithRouter = connect(
    mapStateToProps,
    mapDispatchToProps
)(withRouter(RecoverAccountPrivateKey));

export default RecoverAccountPrivateKeyWithRouter;
